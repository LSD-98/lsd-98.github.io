...
- `netlify/functions/download-report.mjs` — vérifie le JWT, stream le résumé exécutif PDF.
- `netlify/functions/_jwt.mjs` — helpers HMAC-SHA256 (sans dépendance).
- `netlify/functions/assets/Exec_Summary_{FR,EN}.pdf` — résumés exécutifs PDF bundlés hors `static/`.
- `netlify/functions/assets/fund-architecture-report-{fr,en}.pdf` — rapports complets conservés dans le dépôt pour réactivation future, mais non servis actuellement.
...
3. Soumettre le formulaire avec des données de test :
   ...
   - Cliquer sur le bouton English → le résumé exécutif PDF se télécharge, s'ouvre dans un lecteur.
