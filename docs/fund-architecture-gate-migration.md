...
- `netlify/functions/download-report.mjs` — vérifie le JWT, stream le PDF.
- `netlify/functions/_jwt.mjs` — helpers HMAC-SHA256 (sans dépendance).
- `netlify/functions/assets/fund-architecture-report-{fr,en}.pdf` — PDFs bundlés hors `static/`.
...
3. Soumettre le formulaire avec des données de test :
   ...
   - Cliquer sur le bouton English → le PDF se télécharge, s'ouvre dans un lecteur.
